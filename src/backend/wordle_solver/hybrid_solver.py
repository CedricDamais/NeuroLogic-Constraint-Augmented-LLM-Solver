...
class WordleConstraints:
    """Store Wordle game constraints extracted from feedback.
    
    Attributes:
        green: Letters in correct positions {position: letter}
        yellow: Letters in word but wrong positions {position: letter}
        grey: Letters not in the target word
        min_letter_counts: Minimum occurrences of each letter
    """
...
class LanguageAgent:
    """LLM-powered agent for strategic Wordle word selection.
    
    Uses OpenAI's API to make intelligent word choices based on game state,
    candidate words, and information theory principles.
    """
    ...
    def __init__(self, model_name="gpt-4o-mini", api_key=None):
        ...
        Args:
            model_name: OpenAI model to use for suggestions
    ...
    def _calculate_entropy(self, word_list):
        """Calculate the entropy (information content) of a word list.
        
        Args:
            word_list: List of candidate words
            
        Returns:
            float: Entropy value (log2 of list size)
        """
        total = len(word_list)
        if total == 0:
            return 0
    ...
    def _calculate_information_gain(self, word, word_candidates):
        ...
        Args:
            word: Potential guess word
            word_candidates: List of possible target words
            
        Returns:
            float: Expected information gain (entropy reduction)
        """
    ...
    def generate_feedback(self, guess, target_word):
        """Generate Wordle feedback for a guess against target word.
        
        Args:
            guess: Guessed word (string)
            target_word: Target word (string)
            
        Returns:
            List[str]: Feedback list with 'G', 'Y', 'B' for each position
        """
    ...
    def _extract_word(self, content, candidates, word_length):
        """Extract a valid word from LLM response content.
        
        Args:
            content: LLM response text
            candidates: List of valid candidate words
            word_length: Expected word length
            
        Returns:
            str: First valid word found, or first candidate as fallback
        """
